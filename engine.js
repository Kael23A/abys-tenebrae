...
const AT = (() => {
  ...
  function setColor(pilarId) {
    const c = COLORES[pilarId] || COLORES['ecos-del-vacio'];
      ...
      return `
        <div class="historia-card anim-bruma anim-d${Math.min(i+1,4)}" data-id="${h.id}"
          ...
          <div class="card-share">
            <button class="btn-share-card" data-id="${h.id}"
              title="Compartir" onclick="event.stopPropagation()">↗</button>
          </div>
        </div>
      `;
    }).join('');
  ...
  function bindCards(contenedor) {
    ...
    contenedor.querySelectorAll('.btn-share-card').forEach(b =>
      b.addEventListener('click', e => { e.stopPropagation(); abrirShareModal(b.dataset.id); })
    );
  }
  ...
  function initCompartirPagina() {
    ...
    const desc = 'Abys Tenebrae — Portal de narrativa oscura y literatura de penumbra';
    ...
    historias.forEach(h=>{
      (h.comentarios||[]).forEach(c=>{
        ...
        todos.push({...c,
          ...
          pilarNombre:p?.nombre||'', pilarColor:p?.color||COLORES[h.pilar]||'#7ecfc0',
