...
const AT = (() => {
  ...
  function actualizarBtnVolverFlotante() {
    ...
    // Ocultar en home o si no hay historial
    const tieneAtras    = S.historialVistas.length > 0 && S.historialIdx > 0;
  ...
  function initAutoScroll() {
    document.addEventListener('click', e => {
      if (S.vista !== 'lectura') return;
      if (e.target.closest('button,a,#capa-interaccion,.nav-flecha,#btn-biblioteca,#panel-biblioteca,#btn-volver-flotante,#btn-avanzar-flotante,#btn-retorno-top,.bib-item,.bib-item-remove,#btn-tema,#btn-buscar,#btn-aleatorio,#btn-compartir-pagina,#autoscroll-indicator')) return;
      toggleAutoScroll();
    });
    let ty=0, timer=null;
