...
const AT = (() => {
  ...
  function setVista(vista) {
    // Mostrar/ocultar botón inicio header
    const btnInicio = $('#btn-inicio-header');
    if (btnInicio) btnInicio.classList.toggle('visible', vista !== 'home');
  ...
  function initAutoScroll() {
    document.addEventListener('click', e => {
      if (S.vista !== 'lectura') return;
      if (e.target.closest('button,a,#capa-interaccion,.nav-flecha,#btn-biblioteca,#panel-biblioteca,#btn-volver-flotante,#btn-avanzar-flotante,#btn-retorno-top,.bib-item,.bib-item-remove,#btn-tema,#btn-buscar,#btn-aleatorio,#btn-compartir-pagina,#autoscroll-indicator,#btn-inicio-header,#nav-historial')) return;
      toggleAutoScroll();
    });
    let ty=0, timer=null;
  ...
  function bindLogotype() {
    ...
    $('#btn-inicio-header')?.addEventListener('click',()=>{
      detenerAutoScroll(); ocultarNavFlechas();
      history.replaceState(null,'',location.pathname);
      renderHome();
    });
