...
const AT = (() => {
  ...
  // ── Botón Volver Flotante (historial) ──────────────
  function actualizarBtnVolverFlotante() {
    const btn = $('#btn-volver-flotante');
    if (!btn) return;
    // Mostrar si hay historial Y no estamos en home
    const mostrar = S.historialVistas.length > 0 && S.vista !== 'home';
  ...
  function initBtnVolverFlotante() {
    ...
    if (!btn) return;
    btn.addEventListener('click', () => {
      if (!S.historialVistas.length) return;
      const prev = S.historialVistas.pop();
  ...
  function setVista(vista) {
    // Guardar vista anterior en historial (máximo 10)
    if (S.vista && S.vista !== vista) {
      S.historialVistas.push({ vista: S.vista, pilar: S.pilarActivo, sub: S.subgeneroActivo, historia: S.historiaActiva?.id });
      if (S.historialVistas.length > 10) S.historialVistas.shift();
    }
    S.vista = vista;
    actualizarBtnVolverFlotante();
  ...
  function initAutoScroll() {
    document.addEventListener('click', e => {
      if (S.vista !== 'lectura') return;
      if (e.target.closest('button,a,#capa-interaccion,.nav-flecha,#btn-biblioteca,#panel-biblioteca')) return;
      toggleAutoScroll();
    });
    let ty=0, timer=null;
